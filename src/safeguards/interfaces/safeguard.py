...
class Safeguard(VectorActionWrapper, ABC):
    ...
    def __init__(self, env: SafeEnv, regularisation_coefficient: float, **kwargs):
        """
        Args:
            env: A custom secure, pytorch-based environment.
            regularisation_coefficient: The coefficient that scales the regularisation term compared
                                        to the rest of the policy loss.
        """
        super().__init__(env)
    ...
    def actions(self, action: Float[Tensor, "{self.batch_dim} {self.action_dim}"]) \
        ...
        self.interventions += self.batch_dim - torch.isclose(safe_action, action).all(dim=1).sum().item()
        return safe_action
    ...
    def regularisation(self,
                       action: Float[Tensor, "buffer_size {self.num_envs} {self.action_dim}"],
                       safe_action: Float[Tensor, "buffer_size {self.num_envs} {self.action_dim}"]
                       ) -> Float[Tensor, ""]:
        """
        Compute the safeguard-specific regularisation of the policy loss.

        Args:
            action: Policy output.
            safe_action: Safeguard output.
        Returns:
            Regularisation term.
        """
        return self.regularisation_coefficient * torch.nn.MSELoss(reduction="mean")(safe_action, action)
    ...
    def feasibility_constraints(self, action: cp.Expression | np.ndarray) \
        ...
        """
        lower, upper = self.env.action_set.bounds()
        return [
            lower[0, :].cpu().numpy() <= action,
            upper[0, :].cpu().numpy() >= action,
        ]
    ...
    def action_safety_constraints(self,
        ...
        if generator is None:
            constraints = [
                center == safe_action_center + safe_action_generator @ weights,
                cp.norm(weights, "inf") <= 1
            ]
        else:
            mapping = cp.Variable((self.safe_action_gens, generator.shape[1]))
            constraints = [
                generator == safe_action_generator @ mapping,
                safe_action_center - center == safe_action_generator @ weights,
    ...
    def state_safety_constraints(self, action: cp.Expression | np.ndarray) \
        ...
        weights = cp.Variable(self.safe_action_gens)
        mapping = cp.Variable((self.safe_action_gens, next_state_generator.shape[1]))
    ...
    def constraint_parameters(self) -> list[Tensor]:
        parameters = []
        if self.action_constrained:
            parameters += [self.env.safe_action_set().center, self.env.safe_action_set().generator]
        if self.state_constrained:
            parameters += [self.env.safe_state_set().center, self.env.safe_state_set().generator]
